...
export class RecruiterProfileComponent implements OnInit {
  ...
  uploadProfilePicture(): void {
    ...
    this.http.post<{ message: string; recruiter: Recruiter }>(
      `https://jobsphere-backend-zja7.onrender.com/api/upload-recruiter-profile-photo/${this.recruiter.email}`,
      formData
    ).subscribe({
      next: (response) => {
  ...
  saveProfile(): void {
    this.http.put<{ message: string; recruiter: Recruiter }>(
      `https://jobsphere-backend-zja7.onrender.com/api/update-recruiter/${this.recruiter.email}`,
      this.recruiter
    ).subscribe({
      next: (response) => {
